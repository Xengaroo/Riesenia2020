...
public class Nemocnica {
    public static Comparator<Doktor> comparatorDoktorov = new Comparator<Doktor>() {
        @Override
        public int compare(Doktor doktor, Doktor t1) {
            if(doktor.getPrax() > t1.getPrax()) return -1;
            if(doktor.getPrax() < t1.getPrax()) return 1;
            if(doktor.getVek() < t1.getVek()) return -1;
            if(doktor.getVek() > t1.getVek()) return 1;
            return doktor.getMeno().compareTo(t1.getMeno());
        }
    };

    public static Map<String, Set<Doktor>> doktoriNaOddeleniach(Map<Doktor, String> doktori) {
        Map<String, Set<Doktor>> res = new HashMap<>();
        if(doktori == null) return res;
        for (Doktor d : doktori.keySet()) {
            if (res.containsKey(doktori.get(d))) {
                Set<Doktor> pom = res.remove(doktori.get(d));
                pom.add(d);
                res.put(doktori.get(d), pom);
            } else {
                Set<Doktor> pom = new HashSet<>();
                pom.add(d);
                res.put(doktori.get(d), pom);
            }
        }
    ...
    public static Map<Set<Doktor>, List<Doktor>> rozpisyOddeleni(Map<String, Set<Doktor>> oddelenia){
        Map<Set<Doktor>, List<Doktor>> result = new HashMap<>();
        if(oddelenia == null) return result;
        for(String k : oddelenia.keySet()){
            List<Doktor> l = new LinkedList<>(oddelenia.get(k));
            l.sort(comparatorDoktorov);
            result.put(oddelenia.get(k), l);
        }
        return result;
